 ...
 * Run: node scripts/import-triet-utt.cjs
 */
...
async function ensureSchool(shortSchool, fullName) {
...
async function ensureQuestionType() {
...
async function importSubject(subjectPath, subjectConfig) {
    ...
    const subjectData = JSON.parse(fs.readFileSync(subjectDataPath, "utf-8"));
    ...
    const subjectCode = `${subjectConfig.shortSchool}_${subjectConfig.id}`.toUpperCase();
    
    // Find by slug first, then by code
...
async function importChapterQuestions(filePath, subjectId, chapterId) {
    try {
        ...
        for (const q of data.questions) {
            ...
            // Create question with options (QuestionOption has no 'label' field)
                ...
                data: {
                    ...
                    options: {
                        create: q.options.map((opt, idx) => ({
                            content: `${opt.id}. ${opt.content}`, // Include label in content
        ...
        console.error(`    ❌ Error importing questions from ${filePath}:`, error.message);
    }
}

async function importStudyTopics(filePath, subjectId) {
    try {
        ...
        console.log(`  ✅ Topics imported: ${imported}`);
    } catch (error) {
        console.error(`  ❌ Error importing study topics:`, error.message);
    }
}
